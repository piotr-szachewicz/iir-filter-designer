...
public class Chebyshev2IIRDesigner extends ChebyshevIIRDesigner {
	 ...
	 * @param filterOrder the order of the filter to be designed (can be calculated using {@link AbstractIIRDesigner#calculateFilterOrder(org.jdsp.iirfilterdesigner.model.FilterType, double[], double[], double, double, boolean) })
	...
	protected double[] calculateNaturalFrequency(FilterType type, int filterOrder, double[] wp, double[] ws, double gpass, double gstop, boolean analog) {
		...
		double newFrequency = Math.cosh(1.0 / (filterOrder) * SpecialMath.acosh(Math.sqrt((GSTOP - 1.0) / (GPASS - 1.0))));
